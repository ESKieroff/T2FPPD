...
func getUpdates(conn *rpc.Client, args *ClientArgs) error {
	...
	for {
		...
		if err != nil {
			...
			time.Sleep(1 * time.Second) // Tentar novamente em 1 segundo
			continue
		}
...
func main() {
	...
	if len(os.Args) < 3 {
		log.Fatalf("Uso: %s <servidor> <nome>", os.Args[0])
	}
...
func mover(comando rune, conn *rpc.Client) {
	...
	err := updateClientPosition(conn, &clientArgs, dx, dy)
	if err != nil {
		log.Printf("Erro ao mover o cliente: %v", err)
	}
}
